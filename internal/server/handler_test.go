...
type mockStore struct {
	...
	getPhotosEnabledFn       func(ctx context.Context) (bool, error)
...
func (m *mockStore) GetPhotosEnabled(ctx context.Context) (bool, error) {
	if m.getPhotosEnabledFn != nil {
		return m.getPhotosEnabledFn(ctx)
	}
	return false, nil
}

func (m *mockStore) SetPhotosEnabled(ctx context.Context, enabled bool) error {
...
func TestPhotosToggleAPI_Get(t *testing.T) {
	ms := &mockStore{
		getAuthSettingsFn: func(_ context.Context) (*model.AuthSettings, error) {
			return &model.AuthSettings{Enabled: false}, nil
		},
		getPhotosEnabledFn: func(_ context.Context) (bool, error) {
			return false, nil
		},
	}
	srv := newTestServerWithMock(t, ms)
	...
	req := httptest.NewRequest(http.MethodGet, "/api/shelf/photos", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
...
func TestPhotosToggleAPI_Put(t *testing.T) {
	var setCalled bool
	var setVal bool
	ms := &mockStore{
		getAuthSettingsFn: func(_ context.Context) (*model.AuthSettings, error) {
		...
		setPhotosEnabledFn: func(_ context.Context, enabled bool) error {
			setCalled = true
			setVal = enabled
			return nil
		},
	...
	req := httptest.NewRequest(http.MethodPut, "/api/shelf/photos", strings.NewReader(`{"enabled":true}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)

	require.Equal(t, http.StatusOK, w.Code)
	assert.True(t, setCalled, "SetPhotosEnabled should be called")
	assert.True(t, setVal, "SetPhotosEnabled should receive true")
...
func TestPhotosToggleAPI_PutInvalidJSON(t *testing.T) {
	ms := &mockStore{
		getAuthSettingsFn: func(_ context.Context) (*model.AuthSettings, error) {
			return &model.AuthSettings{Enabled: false}, nil
		},
	...
	req := httptest.NewRequest(http.MethodPut, "/api/shelf/photos", strings.NewReader("not json"))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
...
func TestPhotosMiddleware_Disabled(t *testing.T) {
	ms := &mockStore{
		getAuthSettingsFn: func(_ context.Context) (*model.AuthSettings, error) {
			return &model.AuthSettings{Enabled: false}, nil
		},
	...
	handler := srv.photosMiddleware()(next)

	req := httptest.NewRequest(http.MethodGet, "/api/photos/test", nil)
	w := httptest.NewRecorder()
	handler.ServeHTTP(w, req)

	assert.Equal(t, http.StatusNotFound, w.Code)
	assert.False(t, nextCalled, "next handler should not be called when photos disabled")
}

func TestPhotosMiddleware_Enabled(t *testing.T) {
	ms := &mockStore{
		getAuthSettingsFn: func(_ context.Context) (*model.AuthSettings, error) {
			return &model.AuthSettings{Enabled: false}, nil
		},
	...
	assert.Equal(t, http.StatusOK, w.Code)
	assert.True(t, nextCalled, "next handler should be called when photos enabled")
}

func TestSettingsPhotosToggle(t *testing.T) {
	ms := &mockStore{
		getAuthSettingsFn: func(_ context.Context) (*model.AuthSettings, error) {
			return &model.AuthSettings{Enabled: false}, nil
		},
	...
	require.Equal(t, http.StatusOK, w.Code)
	body := w.Body.String()
	assert.Contains(t, body, `id="photos-enabled"`)
	assert.Contains(t, body, "checked")
	assert.Contains(t, body, "Enable Photos")
	assert.Contains(t, body, "Save Photos Settings")
}
