...
func TestPhotoSchema(t *testing.T) {
	s := openTestStore(t)

	// Verify photo table columns
	rows, err := s.conn.Query("PRAGMA table_info(photo)")
	require.NoError(t, err)
	defer rows.Close()

	photoCols := make(map[string]bool)
	for rows.Next() {
		var cid int
	...
	expectedPhotoCols := []string{"id", "uuid", "storage_path", "original_filename", "content_type", "file_size", "uploaded_at"}
	for _, col := range expectedPhotoCols {
		assert.True(t, photoCols[col], "photo table missing column %q", col)
	}

	// Verify item_photo table columns
	rows2, err := s.conn.Query("PRAGMA table_info(item_photo)")
	require.NoError(t, err)
	defer rows2.Close()

	ipCols := make(map[string]bool)
	for rows2.Next() {
		var cid int
		var name, colType string
		var notNull, pk int
		var dflt *string
		require.NoError(t, rows2.Scan(&cid, &name, &colType, &notNull, &dflt, &pk))
		ipCols[name] = true
	}
	require.NoError(t, rows2.Err())

	expectedIPCols := []string{"item_id", "photo_id", "sort_order"}
	for _, col := range expectedIPCols {
		assert.True(t, ipCols[col], "item_photo table missing column %q", col)
	}
}

func TestPhotosEnabledMigration(t *testing.T) {
	s := openTestStore(t)

	var enabled int
	err := s.conn.QueryRow("SELECT photos_enabled FROM shelf LIMIT 1").Scan(&enabled)
	require.NoError(t, err)
	assert.Equal(t, 0, enabled, "photos_enabled should default to 0")
}

func TestGetSetPhotosEnabled(t *testing.T) {
	s := openTestStore(t)
	ctx := context.Background()

	// Initially false
	got, err := s.GetPhotosEnabled(ctx)
	require.NoError(t, err)
	assert.False(t, got)
...
func TestPhotosDisablePreservesData(t *testing.T) {
	s := openTestStore(t)
	ctx := context.Background()
	...
	_, err := s.conn.ExecContext(ctx,
		"INSERT INTO photo (uuid, storage_path, original_filename, content_type, file_size) VALUES (?, ?, ?, ?, ?)",
		"test-uuid", "/tmp/test.jpg", "test.jpg", "image/jpeg", 1024)
	require.NoError(t, err)

	var photoID int64
	require.NoError(t, s.conn.QueryRowContext(ctx, "SELECT id FROM photo WHERE uuid = ?", "test-uuid").Scan(&photoID))

	// Get a container and add an item
	var containerID int64
	require.NoError(t, s.conn.QueryRowContext(ctx, "SELECT id FROM container LIMIT 1").Scan(&containerID))

	result, err := s.conn.ExecContext(ctx,
		"INSERT INTO item (container_id, name) VALUES (?, ?)", containerID, "test-item")
	require.NoError(t, err)
	...
	// Link item to photo
	_, err = s.conn.ExecContext(ctx,
		"INSERT INTO item_photo (item_id, photo_id, sort_order) VALUES (?, ?, 0)", itemID, photoID)
	require.NoError(t, err)
	...
	var photoCount int
	require.NoError(t, s.conn.QueryRowContext(ctx, "SELECT COUNT(*) FROM photo").Scan(&photoCount))
	assert.Equal(t, 1, photoCount, "photo row should be preserved after disabling")

	var ipCount int
	require.NoError(t, s.conn.QueryRowContext(ctx, "SELECT COUNT(*) FROM item_photo").Scan(&ipCount))
	assert.Equal(t, 1, ipCount, "item_photo row should be preserved after disabling")
}
